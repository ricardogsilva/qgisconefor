...
This plugin includes an interface which can be used solely for preparing inputs to use by Conefor as a separate
application.

It also provides a bridge between QGIS and Conefor, allowing you to run Conefor's landscape connectivity 
analysis algorithms from inside QGIS, through the QGIS Processing framework/toolbox. This is a convenient 
environment for landscape and habitat analysis. Conefor algorithms can be directly integrated in more complex 
workflows through models and scripts and use all other GIS algorithms included in the QGIS processing 
framework/toolbox.
...
- **Remove Row/Add Row** buttons: These buttons allow you to remove/add layers to be processed
...
- **Create link vector layer**: when this option is active (together with one distance analysis/query) then the plugin 
  will also generate a map layer with the visual representaion of the node connections. This is generate as a QGIS 
  memory layer and loaded onto the QGIS map canvas. Depending on the number of features in each layer, this option may 
  significantly increase analysis time
...
- **Output directory**: the folder where output Conefor nodes and connections files will be placed. The output file 
  ...
  - connections_centroid-distance_espacios_natura2000.txt
  - connections_edge-distance_espacios_natura2000.txt
